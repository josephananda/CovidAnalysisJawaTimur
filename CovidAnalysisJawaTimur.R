...
# resp$status_code
# identical(resp$status, status_code(resp))
status_code(resp)
...
ggplot(new_cov_jatim, aes(tanggal, kasus_baru)) +
  ...
  labs(
    ...
    subtitle = "Terjadi pelonjakan kasus beberapa kali sepanjang Juni hingga Agustus, namun lonjakan tertinggi terjadi di akhir bulan Agustus",
...
ggplot(cov_jatim_akumulasi_pivot,aes(tanggal,jumlah,colour=(kategori))) +
  ...
  labs(
    ...
    colour = NULL,
    title = "Update Kasus COVID-19 di Jawa Timur",
    caption = "Analysis By: J.A.\nSumber data: covid.19.go.id"
  ) +
  theme_ipsum(
    base_size = 13,
    plot_title_size = 21,
